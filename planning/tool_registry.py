...
class MCPToolRegistry:
    ...
    def __init__(self, mcp_client_or_tools: Any):
        ...
        if isinstance(mcp_client_or_tools, dict):
            ...
            self.tools = {tool.name: tool for tool in mcp_client_or_tools}
        else:
            self.mcp_client = mcp_client_or_tools
            last_tools = getattr(mcp_client_or_tools, "last_tools", {})
            if isinstance(last_tools, list):
                self.tools = {tool.name: tool for tool in last_tools}
