...
OWNERSHIP: this is Person 2's concern (planning-algorithm routing:
Plan-and-Solve vs Tree of Thoughts vs LATS). This file only defines the
interface that decomposition.py / dynamic_decomposition.py call into, plus
a minimal working fallback so the pipeline is runnable end-to-end before
Person 2's planner_selector.py / plan_and_solve.py / tree_of_thoughts.py /
lats.py land.
