...
DB:
  ...
  2. `execute_plan` no longer sends every node to the LLM. A TOOL_CALL node
     ...
     handed off to `planner_router.route_task` (Person 2's concern) instead
...
def decompose_goal(goal: str, llm: BaseChatModel, tool_names: list[str]) -> Plan:
    ...
    generated = llm.with_structured_output(
        ...
        [
            ("system", PLANNER_SYSTEM.format(tool_names="\n".join(f"- {n}" for n in tool_names))),
            (
                "human",
...
async def execute_plan(
    ...
    planner_router: Callable[[Task, dict[str, str], str], Awaitable[str]] | None = None,
    max_workers: int = 4,
) -> dict[str, str]:
    ...
    LLM), each node is dispatched by its `kind`:
      ...
      - PLANNED   -> handed to `planner_router` (PS/ToT/LATS selection)
      - REASONING -> plain LLM call over dependency outputs (upstream path)
    """
    import asyncio

    outputs: dict[str, str] = {}
    for batch in plan.execution_batches():
        # Independent nodes in the same generation run concurrently, mirroring
        # the toolkit's ThreadPoolExecutor batching (async instead of threads
        # since MCP tool calls and langchain's `.ainvoke` are already async).
        async def run_one(task_id: str) -> tuple[str, str]:
            ...
            if task.kind == TaskType.TOOL_CALL:
                result = await _run_tool_node(task, mcp_tools)
            elif task.kind == TaskType.PLANNED:
                if planner_router is None:
                    raise RuntimeError(
                        f"Task {task_id} is 'planned' but no planner_router was supplied"
                    )
                result = await planner_router(task, outputs, plan.goal)
            else:
                result = await _run_reasoning_node(task, plan.goal, context, llm)
            return task_id, result

        results = await asyncio.gather(*(run_one(tid) for tid in batch))
        outputs.update(dict(results))
    return outputs


async def _run_tool_node(task: Task, mcp_tools: dict[str, Any]) -> str:
    tool = mcp_tools.get(task.tool_name)
    if tool is None:
        raise RuntimeError(f"Task {task.id} references unknown tool '{task.tool_name}'")
    # `instruction` for a TOOL_CALL node is expected to carry the concrete
    # arguments the planner extracted, e.g. '{"flight_id": 2}'.
    import json
...
async def _run_reasoning_node(task: Task, goal: str, context: str, llm: BaseChatModel) -> str:
