...
from memory.Fact_extraction import FactExtractorLLM, process_customer_message
...
def handle_final_action(step):
    if step.action == "final_answer":
        print(f"\nAgent: {step.action_input.get('answer', '')}")
        return True

    if step.action == "end_conversation":
        if step.action_input:
            print(f"\nAgent: {step.action_input.get('answer', 'Goodbye!')}")
        else:
            print("\nAgent: Goodbye!")
        return True

    if step.action == "escalate":
        print("\nAgent: Escalating to human support...")
        return True
...
async def run_agent(client, user_input: str, user_id: str = "C001"):
    ...
    episodic_store=episodic_memories[user_id],
    short_term=memory,
    llm=None,  # Router setup uses built-in rules when llm is None
)
    ...
    # Route item once to the episodic store
    ...
    # 1. Fetch relevant long-term semantic memory
    semantic_store = semantic_memories[user_id]
    ...
    if relevant_facts:
        ...
        messages.append(
            SystemMessage(
                content=f"Relevant long-term customer memory:\n{memory_context}"
            )
        )

    # 2. Fetch external domain knowledge via Self-RAG
    rag_info = self_rag(user_input)
    if rag_info:
        messages.append(
            SystemMessage(
                content=f"SYSTEM CONTEXT (Retrieved Knowledge via Self-RAG): {rag_info}"
            )
        )
    ...
    for step_num in range(MAX_STEPS):
        ...
        filtered_messages = zone_based_pruning(memory.get_messages())
